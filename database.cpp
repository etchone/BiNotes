...
void Database::createTablesIfNotExist() {
    ...
    result = query.prepare("CREATE TABLE IF NOT EXISTS settings ("
...
QVector<Memo> Database::filterMemo(const QString& title) const {
    ...
    while (query.next()) {
        auto qid = query.value(0),
             qtitle = query.value(1),
             qcontent = query.value(2);
        Q_ASSERT(qid.canConvert<int>() && qtitle.canConvert<QString>() && qcontent.canConvert<QString>());
        ...
        if (title.isEmpty() || QRegularExpression::fromWildcard(expression).match(title).hasMatch())
...
bool Database::updateMemo(Memo& memo) {
    ...
    case Memo::ContentDirty:
        ...
        break;
    case Memo::BothDirty:
        query_result = query.prepare("UPDATE memo SET title = ?, content = ? WHERE id = ?;");
        Q_ASSERT(query_result);
        query.bindValue(0, memo.title());
...
bool Database::eraseMemo(Memo& memo) {
    ...
    if (query_result && query.numRowsAffected())
        memo.m_id = 0;

    return query_result;
}

QString Database::getSetting(const QString& name, const QString& default_value, bool create_if_not_found) {
    QSqlQuery query(m_db);
    bool result;
...
bool Database::setSetting(const QString& name, const QString& value, bool create_if_not_found) {
