...
MainWindow::MainWindow(QWidget *parent)
  ...
  auto *title_list_view = new QListView(this);
  ...
  auto *name_list_view = new QListView(this);
  ...
  connect(ui->combo_title, &QComboBox::currentTextChanged, this,
          &MainWindow::flushMemos);

  connect(ui->combo_name, &QComboBox::currentIndexChanged, this,
          &MainWindow::onMemoIndexChanged);

  connect(m_filtered_memos_model, &FilteredMemosModel::rowsInserted, this,
          &MainWindow::onMemoChanged);

  connect(ui->edit_content, &QMarkdownTextEdit::textChanged, this,
          &MainWindow::onMemoContentChanged);

  connect(QApplication::instance(), &QApplication::aboutToQuit, this,
          &MainWindow::saveContent);

  connect(Actions::getInstance(), &Actions::hideWindowTriggered, this,
          &MainWindow::hide);
  connect(Actions::getInstance(), &Actions::showWindowTriggered, this,
          &MainWindow::show);

  connect(Actions::getInstance(), &Actions::hideSettingsToggled,
          [this](bool checked) { toggleSettingsVisibility(!checked); });

  connect(Actions::getInstance(), &Actions::removeMemoTriggered, this,
          &MainWindow::removeCurrentMemo);

  connect(Actions::getInstance(), &Actions::monitorForegroundToggled, this,
          &MainWindow::toggleForegroundMonitorTimer);

  connect(Actions::getInstance(), &Actions::flushTitlesTriggered,
          m_active_window_model, &ActiveWindowModel::updateTitles);

  connect(this, &MainWindow::memoEditableChanged, ui->edit_content,
          &QMarkdownTextEdit::setVisible);
  connect(this, &MainWindow::memoEditableChanged, Actions::getInstance(),
          &Actions::setRemoveMemoActionEnabled);
  ...
  //            this, &MainWindow::saveWindowGeometryToDatabase);
  connect(this, &MainWindow::loadSettingsFromDatabase, this,
          &MainWindow::loadWindowGeometryFromDatabase);
  connect(this, &MainWindow::loadSettingsFromDatabase, Actions::getInstance(),
          &Actions::loadCheckStatesFromDatabase);

  connect(Database::getInstance(), &Database::aboutToClose, this,
          &MainWindow::saveWindowGeometryToDatabase);
  connect(Database::getInstance(), &Database::aboutToClose,
          Actions::getInstance(), &Actions::saveCheckStatesToDatabase);

  connect(Actions::getInstance(), &Actions::autorunToggled, this,
          &MainWindow::setRunOnSystemStartup);
...
void MainWindow::flushMemos() {
  ...
  bool flushed =
      m_filtered_memos_model->setFilter(ui->combo_title->currentText());
  if (!flushed) return;
  ...
  if (row != count - 1) ui->combo_name->setCurrentIndex(row);
}
...
void MainWindow::onMemoChanged() {
  ...
  if (m_current_row == m_filtered_memos_model->rowCount() - 1) {
    ...
    ui->edit_content->setPlainText(
        m_filtered_memos_model
            ->data(m_filtered_memos_model->index(m_current_row, 1))
            .toString());
  }
...
void MainWindow::saveContent() {
  ...
  if (m_current_memo != nullptr && m_current_memo->isDirty()) {
    ...
    // But since we do not display content at any other place, this way is
    // acceptable
    m_current_memo->setContent(ui->edit_content->toPlainText());
...
void MainWindow::toggleForegroundMonitorTimer(bool enabled) {
  ...
  if (enabled) {
    ...
    connect(m_flush_timer, &QTimer::timeout, m_active_window_model,
            &ActiveWindowModel::updateTitles);
    m_flush_timer->start();
  } else {
    m_flush_timer->disconnect(this, nullptr);
    m_flush_timer->stop();
...
void MainWindow::removeCurrentMemo() {
  ...
  if (m_current_memo != nullptr) {
    ...
    //  -> MainWindow::onMemoIndexChanged
    //  -> MainWindow::saveContent
    // if m_current_memo is not null, it will try to access a memo that's
    // deleted
    m_current_memo = nullptr;
    qDebug() << m_current_memo;
