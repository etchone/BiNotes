 ...
 public:
    explicit FilteredMemosModel(QObject* parent = nullptr)
        ...
        connect(Database::getInstance(), &Database::aboutToClose,
                this, &FilteredMemosModel::clearAndSync);
...
#pragma region Necessary implementation
    ...
    QVariant headerData(int section,
        ...
        return orientation == Qt::Horizontal
                   ? QVariant(section)
                   : (section == 0
                          ? QVariant(tr("title", "databaseTitle"))
                          : QVariant(tr("content", "databaseContent")));
    }

    int rowCount(const QModelIndex& parent = QModelIndex()) const final {
        return parent.isValid() ? 0 : m_filtered_memos.count();
    }
...
#pragma region Editable items
    ...
    bool setData(const QModelIndex &index, const QVariant &value, int role = Qt::EditRole) {
        if (index.column() > 1 || index.row() >= rowCount() || !value.canConvert<QString>() || role != Qt::EditRole)
            return false;
...
private slots:
    void clearAndSync() {
        if (rowCount() > 1) {
            ...
            for (int count = m_filtered_memos.size() - 1; count; --count) {
 ...
 public slots:
    ...
    bool setFilter(const QString& glob_expression) {
        if (glob_expression == m_filter_expression)
            return false;
        m_filter_expression = glob_expression;
        auto &&list = Database::getInstance()->filterMemo(glob_expression == "(All)" ? "" : glob_expression);
...
private:
    ...
    const Memo& addMemo(const QString& title) {
        ...
        auto &memo = *m_filtered_memos.insert((qsizetype)(m_filtered_memos.count() - 1), memo_r);
        endInsertRows();
        return memo;
    }
