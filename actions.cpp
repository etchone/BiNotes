...
Actions::Actions()
    : m_window_hidden(false),
      ...
      m_autorun(false) {
  connect(this, &Actions::quitProgramTriggered, QApplication::instance(),
          &QApplication::quit);
}

QAction *Actions::createDefaultAction(const QString &text,
                                      void (Actions::*slot)()) {
...
QAction *Actions::createControllableAction(const QString &text, bool *enabled,
                                           void (Actions::*slot)()) {
  auto *action = new QAction(text, this);
...
QAction *Actions::createCheckableAction(const QString &text, bool *stored_value,
...
QAction *Actions::createDefaultActionWithShortcut(const QString &text,
...
void Actions::verifyRemoveMemo() {
  QMessageBox mbox(
      QMessageBox::Warning, tr("Confirm"),
      tr("Are you sure to remove current memo?"),
      QMessageBox::Yes | QMessageBox::No, MainWindow::getInstance(),
      Qt::Dialog | Qt::MSWindowsFixedSizeDialogHint | Qt::WindowStaysOnTopHint);
  mbox.setDefaultButton(QMessageBox::No);
  int ret = mbox.exec();

  if (ret == QMessageBox::Yes) emit removeMemoTriggered();
}

void Actions::verifyQuitProgram() {
  QMessageBox mbox(
      QMessageBox::Warning, tr("Confirm"), tr("Are you sure to quit?"),
      QMessageBox::Yes | QMessageBox::No, MainWindow::getInstance(),
      Qt::Dialog | Qt::MSWindowsFixedSizeDialogHint | Qt::WindowStaysOnTopHint);
  mbox.setDefaultButton(QMessageBox::No);
  int ret = mbox.exec();
...
QAction *Actions::hideSettingsAction() {
  return createCheckableAction(tr("Hide settings"), &m_settings_hidden,
                               &Actions::hideSettingsToggled);
}
QAction *Actions::removeMemoAction() {
  return createDefaultActionWithShortcut(tr("Remove current memo"),
                                         QKeySequence(Qt::Key_Delete),
                                         &Actions::removeMemoTriggered);
}
QAction *Actions::flushTitlesAction() {
  return createDefaultAction(tr("Flush titles"),
                             &Actions::flushTitlesTriggered);
}
QAction *Actions::monitorForegroundAction() {
  return createCheckableAction(tr("Monitor foreground window"),
                               &m_monitor_foreground,
                               &Actions::monitorForegroundToggled);
}
QAction *Actions::autoRunAction() {
  return createCheckableAction(tr("Run on startup"), &m_autorun,
                               &Actions::autorunToggled);
}
