...
WindowHandleButton::WindowHandleButton(QWidget *parent)
    : QToolButton(parent), m_action(ActionState::Idle), m_last_pos() {
...
void WindowHandleButton::mousePressEvent(QMouseEvent *e) {
  ...
  //    e->buttons().testFlags(Qt::LeftButton | Qt::RightButton)) {
  //        QApplication::quit();
  //        return;
  //    }
...
void WindowHandleButton::mouseReleaseEvent(QMouseEvent *e) {
  ...
  if (e->button() == Qt::RightButton &&
      e->buttons().testAnyFlag(Qt::LeftButton)) {
  ...
  if (e->buttons() == Qt::NoButton) m_last_pos = QPoint();
}

void WindowHandleButton::mouseMoveEvent(QMouseEvent *e) {
  QToolButton::mouseMoveEvent(e);

  if (m_action == ActionState::Idle) return;
  ...
  if (m_action == ActionState::Move) {
    ...
    size +=
        {current_pos.x() - m_last_pos.x(), m_last_pos.y() - current_pos.y()};
    win->resize(size);
